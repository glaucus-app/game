...
def test_entropy_system_compute_example():
    world = WorldState(
        ...
        coalitions=[
            Coalition(id="c1", members=["a1", "a2"], state=CoalitionState.PACT, formed_at_turn=40)
        ],
        information_asymmetry=0.81,
    )
    for agent_id in ["a1", "a2", "a3", "a4"]:
        world.agents[agent_id] = AgentState(agent_id=agent_id)
    ...
    assert math.isclose(s_e.composite, 0.572, abs_tol=0.01)
...
def test_trust_investment_intervention():
    ...
    world.agents["a1"] = AgentState(agent_id="a1")
    system = EntropySystem()
