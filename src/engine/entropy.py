...
def _agent_count(world: WorldState) -> int:
    agents: set[str] = set()
    for coalition in world.coalitions:
        agents.update(coalition.members)
    for agent, connections in world.trust_graph.items():
        agents.add(agent)
        agents.update(connections)
    return len(agents)
...
class EntropySystem:
    ...
    def compute_coalition_entropy(self, world: WorldState) -> float:
        ...
        total_agents = _agent_count(world)
        if total_agents == 0:
            return 0.0
        fraction = agents_in_stable / total_agents
        ke = 1.0 - (fraction ** 2)
    ...
    def compute_info_asymmetry_entropy(self, world: WorldState) -> float:
        if world.information_asymmetry <= 0:
            return 0.0
        n_states = max(2, _agent_count(world) + 1)
