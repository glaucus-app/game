...
from src.models import Coalition, CoalitionState, EntropyConfig, EntropySnapshot, MaterialEntropy, SocialEntropy, WorldState
...
class EntropySystem:
    ...
    def compute_coalition_entropy(self, world: WorldState) -> float:
        if not world.coalitions or not world.agents:
            return 0.0
        stable_coalitions = [
            c for c in world.coalitions
            if c.state in (CoalitionState.PACT, CoalitionState.RFORMATION)
        ]
        agents_in_stable = sum(len(c.members) for c in stable_coalitions)
        fraction = agents_in_stable / len(world.agents)
        ke = 1.0 - (fraction ** 2)
    ...
    def compute_info_asymmetry_entropy(self, world: WorldState) -> float:
        if world.information_asymmetry <= 0:
            return 0.0
        n_states = max(2, len(world.agents) + 1)
